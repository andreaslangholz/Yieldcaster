...
from src.models import makeDataset, neuralNet, train_k_epochs, criterion_rmse, calc_loss_rmse
import warnings
...
x_train = df_test.loc[:, df_test.columns!=target]
...
# Predict rolling avg
df_roll_train = baseline_rolling(df_train, 3).drop('yield', axis=1, inplace = True)
df_roll_test = baseline_rolling(df_test, 3).drop('yield', axis=1, inplace = True)
...
df_results_train.to_csv("C:\\Users\\Andreas Langholz\\Yieldcaster\\data\\outputs\\hist_errors\\model1err_train.csv")
df_results_test.to_csv("C:\\Users\\Andreas Langholz\\Yieldcaster\\data\\outputs\\hist_errors\\model1err_test.csv")
